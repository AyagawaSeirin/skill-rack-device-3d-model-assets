...
- 使用 PDF 技能读取官方白皮书、规格书、Quick Specs、安装和维护手册，提取参数并渲染相关页面。
- 使用图像查看能力逐张识别网页图片、PDF 页面和用户素材中的型号、视角、配置与细节。
- 官方页面资料不足时，使用 Browser 打开动态图库，并继续搜索经销商、购物网站、二手设备、拍卖和评测素材。
- 建立逐部件特征清单，精确记录硬盘、节点、端口、风扇、电源、把手和面板数量及布局。
- 根据官方正面、背面、左右侧面、顶部、底部及多角度资料制作六面素材。
- 六面素材必须分别调用 GPT `imagegen`，把网上图片和 PDF 页面作为绑定参考生成透明背景 PNG，不能只做裁剪抠图。
- 允许并要求生成新图片，但真实服务器照片必须锁定设备身份、结构、材质、颜色和摄影风格；不能生成成另一种 CG、插画、游戏贴图或“美化产品图”风格。
- 如果已经找到正确正面/背面实拍图，它必须是该面的最高优先级主参考，旧 AI 图和 GLB 预览不能覆盖它。
- 底部资料在完整搜索后仍缺失时，允许使用经检查的通用底面参考进行 GPT 生图兜底并明确标注。
...
简化可见结构或生成插画风素材。读取官方 PDF、检查网页/PDF 图片，并以这些
素材为参考逐面调用 imagegen 生成透明 PNG。生成图必须保持真实照片的材质、
颜色、表面纹理和摄影风格，不得换成 CG/插画风；官方 3D 文件只作为备用选项。
```
...
PDF 页码/图号、图像识别记录、imagegen 提示词与输入角色、六面透明素材、
标准 GLB、Web GLB 和同机位外观对照报告。只有底面可使用已披露兜底素材。
```
...
- 正面和背面不能代替完整六面资料。
- 官方搜索没有结果时，必须继续检查动态网页和第三方/购物网站素材，不能过早停止。
- PDF 文字提取不能代替页面渲染和图片识别；网页标题、文件名和缩略图也不能代替查看原图。
- 每个最终六面 PNG 都必须经过内置 imagegen 参考生图并保留透明背景，不能只做本地裁剪、抠图或透视变形。
- “使用生图”不等于允许换风格：生成结果必须和主实拍参考保持同一种真实摄影质感、金属/塑料材质、颜色、磨损、光照和阴影特征。
- 已有正确实拍图时，必须使用 `SOURCE_LOCKED_GENERATION`；AI 历史图、旧 GLB 贴图和预览图不能成为主参考。
- 仅说“photorealistic/照片级”不够，必须明确禁止 CGI 清理、重新打光、美化、平滑材质、降噪抹纹理、调色、人工对称和插画化。
- 不能用通用机箱或相似型号填补正面、背面、左右侧面和顶部；只有底面搜索穷尽后可以使用保守的通用兜底。
- 底面兜底必须通过 imagegen 从经检查的参考图生成，不能镜像顶部或编造标签、孔洞、脚垫和导轨，并标记 `PASS_WITH_BOTTOM_FALLBACK`。
